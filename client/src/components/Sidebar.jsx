...
const Sidebar = () => {
    ...
    return (
        <div className={`bg-[#818582]/10 h-full p-5 rounded-r-xl flex flex-col overflow-y-auto
            ...
            <div className="flex flex-col flex-1 overflow-y-auto min-h-0">
                {filteredUsers.map((user, index) => (
                    <div onClick={() => { setSelectedUser(user); setUnseenMessages(prev => ({ ...prev, [user._id]: 0 })) }}
                        key={user._id || index} className={`relative flex items-center gap-2 p-2 pl-4 rounded
